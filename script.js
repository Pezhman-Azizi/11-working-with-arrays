...
(sarahDog.curFood > sarahDog.recFood)? console.log(`sarah's dog is eating too much`) : console.log(`sarah's dog is eating too little`);
...
const exactly = dogs.some(dog => dog.recFood === dog.curFood)
console.log('exactly', exactly);

// 6. Log to the console whether ALL of the dogs are eating an OKAY amount of food (just true or false)
const okay = dogs.every(dog => dog.curFood > dog.recFood*0.9 && dog.curFood < dog.recFood*1.9)
console.log('okay', okay);

// 7. Create an array containing the dogs that are eating an OKAY amount of food (try to reuse the condition used in 6.)
const dogsOk = dogs
.filter(dog => dog.curFood > dog.recFood*0.9 && dog.curFood < dog.recFood*1.9)
console.log(dogsOk);
...
const groupedDogsByOwners = Object.groupBy(dogs, dog=>{
  if(dog.owners.length >=2 ) return 'multiple owner dogs';
  if(dog.owners.length === 1) return 'one owner dogs'
})
